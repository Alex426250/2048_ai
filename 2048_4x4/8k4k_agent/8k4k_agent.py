...
def append_training_data(data_path, episodes, avg_score, merged_16384):
    with open(data_path, "a", encoding="utf-8") as f:
        f.write(f"{episodes} {avg_score:.2f} {int(merged_16384)}\n")

class TDAgent:
    def __init__(self, alpha=0.001, dll_path="8k4k_agent.dll"):
                      
        script_dir = os.path.dirname(os.path.abspath(__file__))
        full_dll_path = os.path.join(script_dir, dll_path)
        
        if not os.path.exists(full_dll_path):
            raise FileNotFoundError(f"找不到编译好的 C++ 动态库: {full_dll_path}。\n请先运行: g++ -O3 -shared -fPIC 8k4k_agent.cpp -o 8k4k_agent.dll")
    ...
    def save_model(self, path="8k4k_model.bin"):
        encoded_path = path.encode('utf-8')
        return self.lib.save_model(self.agent_ptr, encoded_path)

    def load_model(self, path="8k4k_model.bin"):
        encoded_path = path.encode('utf-8')
...
if __name__ == "__main__":
    ...
    model_path = "8k4k_model.bin"
    data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data_8k4k.txt")
    ...
    try:
        ...
        while total_episodes < target_episodes:
            ...
            print(
                ...
                f"平均: {avg_score:.2f} | "
                f"合成16384+8192次数: {int(merged_8192)}"
            )
