...
class TDAgent:
    def __init__(self, alpha=0.001, dll_path="16k8k4k_agent.dll"):
        ...
        if not self.agent_ptr:
            raise RuntimeError("C++ init_agent 返回空指针，动态库初始化失败")

    def __del__(self):
        self.close()

    def close(self):
        if getattr(self, 'agent_ptr', None):
    ...
    def train(self, episodes):
        ...
        merged_2048 = ctypes.c_int(0)
        self.lib.train(
            self.agent_ptr, 
            episodes, 
            ctypes.byref(avg_score), 
            ctypes.byref(merged_8192), 
            ctypes.byref(merged_2048),
        )
        return avg_score.value, merged_8192.value, merged_2048.value
    ...
    def save_model(self, path="16k8k4k_model.bin"):
        encoded_path = path.encode('utf-8')
        return self.lib.save_model(self.agent_ptr, encoded_path)

    def load_model(self, path="16k8k4k_model.bin"):
        encoded_path = path.encode('utf-8')
...
if __name__ == "__main__":
    ...
    try:
        total_episodes = 0
        target_episodes = 10_000_000
        batch_size = 10_000
        data_path = "data_16k8k4k.txt"

        while total_episodes < target_episodes:
                                            
            remaining = target_episodes - total_episodes
            run_episodes = batch_size if remaining >= batch_size else remaining
            avg_score, merged_8192, merged_2048 = agent.train(run_episodes)
            total_episodes += run_episodes
            print(f"目前已对弈 {total_episodes} 局, 本批次统计 -> 平均: {avg_score:.2f} | 再合成2048成功次数: {int(merged_2048)} | 合成8192成功次数: {int(merged_8192)}")

            with open(data_path, "a", encoding="utf-8") as f:
                f.write(f"{total_episodes} {avg_score:.2f} {int(merged_2048)} {int(merged_8192)}\n")

            if total_episodes % 100000 == 0:
                agent.save_model(model_path)
                print(f" >>> [自动备份] 模型权重数据已安全快照保存至硬盘。")

        print("已达到预定训练局数 10000000，正在保存模型...")
        agent.save_model(model_path)
        print("模型已保存，训练完成。")
