...
struct Config {
	...
	int max_sum = 0;
	int threads = 1;
	string output = "table_2x4_512.bin";
};
...
struct Tables {
	...
	vector<LineMove<2>> left2;
	vector<LineMove<2>> right2;
	vector<LineMove<4>> left4;
	vector<LineMove<4>> right4;
};
...
Config parse_args(int argc, char **argv) {
	...
	if (key_space > UINT32_MAX) throw runtime_error("key space exceeds uint32_t; this builder is intended for 256/512 2x4 tables");
	cfg.key_space = static_cast<uint32_t>(key_space);
	cfg.max_sum = CELLS * (1 << cfg.max_exp);

	if (cfg.output == "table_2x4_512.bin" && cfg.target != DEFAULT_TARGET) cfg.output = "table_2x4_" + to_string(cfg.target) + ".bin";

	return cfg;
}

template <size_t L>
uint16_t line_code(const Tables &tab, const array<int, L> &d) {
	uint32_t key = 0;
	for (size_t i = 0; i < L; ++i) key += static_cast<uint32_t>(d[i]) * tab.pow_base[i];
	return static_cast<uint16_t>(key);
}

uint16_t line_code2(const Tables &tab, int a, int b) {
	return static_cast<uint16_t>(a + tab.cfg.base * b);
}

uint16_t line_code4(const Tables &tab, int a, int b, int c, int d) {
	return static_cast<uint16_t>(a + tab.cfg.base * b + tab.pow_base[2] * c + tab.pow_base[3] * d);
}

template <size_t L>
array<int, L> decode_line(const Tables &tab, uint16_t code) {
	array<int, L> d{};
	for (size_t i = 0; i < L; ++i) {
		d[i] = code % tab.cfg.base;
		code = static_cast<uint16_t>(code / tab.cfg.base);
    }
	return d;
}
...
uint16_t reverse_line(const Tables &tab, uint16_t code) {
	auto d = decode_line<L>(tab, code);
	reverse(d.begin(), d.end());
	return line_code(tab, d);
}

template<size_t L>
LineMove<L> build_left_move(const Tables &tab, uint16_t code) {
	auto in = decode_line<L>(tab, code);
	array<int, L> vals{};
	int n = 0;
	...
	for (int i = 0; i < n;) {
		if ( i + 1 < n && vals[i] == vals[i + 1]) {
			int merged = vals[i] + 1;
			if (merged >= tab.cfg.target_exp) return {0, {}, true, true};
			out[m++] = merged;
			i += 2;
		} else out[m++] = vals[i++];
	}

	uint16_t out_code = line_code(tab, out);
	LineMove<L> lm;
	lm.out = out_code;
	lm.changed = out_code != code;
	for (size_t i = 0; i < L; ++i) lm.d[i] = static_cast<uint8_t>(out[i]);
	return lm;
}

template <size_t L>
void build_line_tables(Tables &tab, vector<LineMove<L>> &left, vector<LineMove<L>> &right) {
	int line_count = static_cast<int>(pow_u64(static_cast<uint64_t>(tab.cfg.base), L));
	left.resize(line_count);
	right.resize(line_count);

	for (int code = 0; code < line_count; ++code) {
		LineMove<L> lm = build_left_move<L>(tab, static_cast<uint16_t>(code));
		left[code] = lm;

		uint16_t reversed = reverse_line<L>(tab, static_cast<uint16_t>(code));
		LineMove<L> left_reversed = build_left_move<L>(tab, reversed);

		LineMove<L> rm;
		rm.won = left_reversed.won;
		rm.changed = left_reversed.changed;
		if (!rm.won) {
			rm.out = reverse_line<L>(tab, left_reversed.out);
			rm.changed = rm.out != code;
			for (size_t i = 0; i < L; ++i) rm.d[i] = left_reversed.d[L - 1 - i];
		}
		right[code] = rm;
	}
}
...
void build_tables(Tables &tab) {
	...
	build_line_tables<2>(tab, tab.left2, tab.right2);
	build_line_tables<4>(tab, tab.left4, tab.right4);
}
...
bool move_board_after(const Tables &tab, const array<uint8_t, CELLS> &in, char dir, array<uint8_t, CELLS> &out, bool &won) {
	...
	if (dir == 'L') {
		const auto &m = tab.left4;
		const auto &a = m[line_code4(tab, in[0], in[1], in[2], in[3])];
		if (a.won) return won = true;
		const auto &b = m[line_code4(tab, in[4], in[5], in[6], in[7])];
		if (b.won) return won = true;
		for (int i = 0; i < 4; ++i) out[i] = a.d[i], out[4 + i] = b.d[i];
		return a.changed || b.changed;
	} else if (dir == 'R') {
		const auto &m = tab.right4;
		const auto &a = m[line_code4(tab, in[0], in[1], in[2], in[3])];
		if (a.won) return won = true;
		const auto &b = m[line_code4(tab, in[4], in[5], in[6], in[7])];
		if (b.won) return won = true;
		...
		const auto &m = tab.left2;
		const auto &a =  m[line_code2(tab, in[0], in[4])];
		if(a.won) return won = true;
		const auto &b =  m[line_code2(tab, in[1], in[5])];
		if(b.won) return won = true;
		const auto &c =  m[line_code2(tab, in[2], in[6])];
		if(c.won) return won = true;
		const auto &d =  m[line_code2(tab, in[3], in[7])];
		if(d.won) return won = true;
		out[0] = a.d[0]; out[4] = a.d[1];
		out[1] = b.d[0]; out[5] = b.d[1];
		out[2] = c.d[0]; out[6] = c.d[1];
		out[3] = d.d[0]; out[7] = d.d[1];
		return a.changed || b.changed || c.changed || d.changed;
	} else {
		const auto &m = tab.right2;
		const auto &a =  m[line_code2(tab, in[0], in[4])];
		if(a.won) return won = true;
		const auto &b =  m[line_code2(tab, in[1], in[5])];
		if(b.won) return won = true;
...
public:
	explicit ParallelRunner(int requested_threads) : threads_(max(1, requested_threads)) {
...
float calc_p_value(const Tables &tab, const float *r_raw, uint32_t p_key) {
	...
	auto take2 = [&](const LineMove<4> &a, const LineMove<4> &b, uint32_t key) {
		if (a.won || b.won) {
			best = 1.0f;
			return;
		}
	...
	auto take4 = [&](const LineMove<2> &a, const LineMove<2> &b, const LineMove<2> &c, const LineMove<2> &d4, uint32_t key) {
		if (a.won || b.won || c.won || d4.won) {
			best = 1.0f;
			return;
		}
		if (!(a.changed || b.changed || c.changed || d4.changed)) return;
		float v = r_raw[key];
		if (v > best) best = v;
	};
	...
	{
		const auto &m = tab.left4;
		const auto &a = m[line_code4(tab, d[0], d[1], d[2], d[3])];
		const auto &b = m[line_code4(tab, d[4], d[5], d[6], d[7])];
		take2(a, b, static_cast<uint32_t>(a.out) + static_cast<uint32_t>(b.out) * p[4]);
	}
	{
		const auto &m = tab.right4;
		const auto &a = m[line_code4(tab, d[0], d[1], d[2], d[3])];
	...
	{
		const auto &m = tab.left2;
		const auto &a = m[line_code2(tab, d[0], d[4])];
		const auto &b = m[line_code2(tab, d[1], d[5])];
		const auto &c = m[line_code2(tab, d[2], d[6])];
		const auto &e = m[line_code2(tab, d[3], d[7])];
		uint32_t key = static_cast<uint32_t>(a.d[0]) * p[0] + static_cast<uint32_t>(b.d[0]) * p[1]
				       + static_cast<uint32_t>(c.d[0]) * p[2] + static_cast<uint32_t>(e.d[0]) * p[3]
				       + static_cast<uint32_t>(a.d[1]) * p[4] + static_cast<uint32_t>(b.d[1]) * p[5]
				       + static_cast<uint32_t>(c.d[1]) * p[6] + static_cast<uint32_t>(e.d[1]) * p[7];
		take4(a, b, c, e, key);
	}
	{
		const auto &m = tab.right2;
...
void export_model(const Tables &tab, const FlatBuckets &keys, const float *raw) {
	...
	ModelHeader h{};
	memcpy(h.magic, "2X4RQ71", 7);
	h.version = 1, h.rows = 2, h.cols = 4, h.cells = CELLS;
