...
public:
    ...
    static constexpr int LUT_GROUP_COUNT = 10;
    static constexpr int LUT_RADIX = 11;
    ...
    void init_random_snapshots() {
        ...
        while (snapshot_pool.size() < 1000) {
            vector<int> b(12, 0);
            int mode = rand() % 10;
            if (mode < 9) b[0] = 4096, b[4] = 1024;
            else {
                int pos1 = rand() % 12;
                int pos2 = rand() % 11;
                if (pos2 >= pos1) pos2++;
                b[pos1] = 4096, b[pos2] = 1024;
            }
            for (int i = 0; i < 12; i++) if (!b[i]) b[i] = random_small_tile();
            snapshot_pool.push_back(b);
        }
    }

    void train(int episodes, double* avg_score, int* merged_2048_out) {
        ...
        for(int ep = 0; ep < episodes; ep++) {
            ...
            while(!g.is_over()) {
                ...
                if (best_m == -1) break;
                if (reached_2048) break;
                if (has_old) update_weights(old_after, best_v);
                g.move(best_m);
                for (int i=0; i<12; i++) if (((g.board >> (i * 4)) & 0xF) == 11) reached_2048 = true;
                old_after = best_after;
                has_old = true;
            }
            if (reached_2048) merged_2048_count += 1;
            if (has_old && !reached_2048) update_weights(old_after, 0);
            total_score += g.score;
...
extern "C" {
    ...
    __declspec(dllexport) void train(void* agent, int episodes, double* avg_s, int* merged_2048_s) {
        ((TDAgentCpp*)agent)->train(episodes, avg_s, merged_2048_s);
    }
