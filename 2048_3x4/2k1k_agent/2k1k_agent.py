...
def append_training_data(data_path, episodes, avg_score, merged_2048):
    with open(data_path, "a", encoding="utf-8") as f:
        f.write(f"{episodes} {avg_score:.2f} {int(merged_2048)} \n")

class TDAgent:
    def __init__(self, alpha=0.001, dll_path="2k1k_agent.dll"):
                      
        script_dir = os.path.dirname(os.path.abspath(__file__))
        full_dll_path = os.path.join(script_dir, dll_path)
        
        if not os.path.exists(full_dll_path):
            raise FileNotFoundError(f"找不到编译好的 C++ 动态库: {full_dll_path}。\n请先运行: g++ -O3 -shared -fPIC 2k1k_agent.cpp -o 2k1k_agent.dll")
    ...
    def train(self, episodes):
        ...
        self.lib.train(self.agent_ptr, episodes, ctypes.byref(avg_score), ctypes.byref(merged_2048))
        return avg_score.value, merged_2048.value

    def get_best_move(self, grid_2d):
        """被 Python 的 GUI 或其它脚本调用：传入 3x4 list，返回最优方向 0,1,2,3"""
        flat_grid = []
        for r in range(3):
            for c in range(4):
                flat_grid.append(grid_2d[r][c])
                
        c_array = (ctypes.c_int * 12)(*flat_grid)
        move = self.lib.get_best_move(self.agent_ptr, c_array)
        return move if move != -1 else None

    def save_model(self, path="2k1k_model.bin"):
        encoded_path = path.encode('utf-8')
        return self.lib.save_model(self.agent_ptr, encoded_path)

    def load_model(self, path="2k1k_model.bin"):
        encoded_path = path.encode('utf-8')
...
if __name__ == "__main__":
    ...
    model_path = "2k1k_model.bin"
    if agent.load_model(model_path):
        print(f"已加载现有模型: {model_path}，继续进化...")
    else:
        print("未找到已有模型，从头开始训练...")
        
    data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data_2k1k.txt")
    ...
    try:
        ...
        while total_episodes < target_episodes:
            ...
            avg_score, merged_2048= agent.train(batch)
            total_episodes += batch
            append_training_data(data_path, total_episodes, avg_score, merged_2048)
            print(
                f"目前已对弈 {total_episodes} 局, 本批次得分统计 -> "
                f"平均: {avg_score:.2f} | 合成2048次数: {int(merged_2048)} "
            )
