...
constexpr uint8_t SYM_POS[8][CELLS] = {
	{0, 1, 2, 3, 4, 5, 6, 7, 8},
	{2, 5, 8, 1, 4, 7, 0, 3, 6},
	{8, 7, 6, 5, 4, 3, 2, 1, 0},
	{6, 3, 0, 7, 4, 1, 8, 5, 2},
...
struct Table {
	...
	void load(const string &file, uint32_t target) {
		...
		if (memcmp(h.magic, "3X3RQ71", 7) != 0 || h.version != 1 || h.rows != 3 || h.cols != 3 || h.cells != CELLS || 
			h.target != target || h.prob_type != 3 || h.file_size != mf.size) throw runtime_error("bad 3x3 R model: " + file);
		pow_base[0] = 1;
...
uint8_t hex_exp(char c) {
	...
	if ('A' <= c && c <= 'F') return uint8_t(c - 'A' + 10);
	throw runtime_error("board must be 9 hex digits");
}

bool move_line(const array<uint8_t, CELLS> &in, const array<int, 3> &pos, array<uint8_t, CELLS> &out) {
	array<uint8_t, 3> vals{}, line{};
	int n = 0, w = 0;
	...
	bool changed = false;
	for (int i = 0; i < 3; ++i) out[pos[i]] = line[i], changed = changed || out[pos[i]] != in[pos[i]];
	return changed;
}
...
bool move_board(const array<uint8_t, CELLS> &b, char dir, array<uint8_t, CELLS> &a) {
	...
	if (dir == 'L' || dir == 'R') {
		for (int r = 0; r < 3; ++r) {
			if (dir == 'L') changed = move_line(b, {r * 3, r * 3 + 1, r * 3 + 2}, a) || changed;
			else changed = move_line(b, {r * 3 + 2, r * 3 + 1, r * 3}, a) || changed;
		}
	} else {
		for (int c = 0; c < 3; ++c) {
			if (dir == 'U') changed = move_line(b, {c, c + 3, c + 6}, a) || changed;
			else changed = move_line(b, {c + 6, c + 3, c}, a) || changed;
		}
	}
	return changed;
}
...
void usage(const char *argv0) {
	cerr << "Usage:\n" << "  " << argv0 << " 512|1024 HEX\n" << "  " << argv0
		<< " --model FILE 512|1024 HEX\n" << "HEX: 0=empty, 1=2, ..., 9=512, A=1024\n";
}
...
extern "C" EXPORT int init_model(const char* model_path, int target) {
    if (target != 512 && target != 1024) return -1;
    try {
...
extern "C" EXPORT int query_probs(const char* board_hex, int target, double out_probs[4]) {
    ...
    try {
        ...
        if (hex.size() == 11 && hex[0] == '0' && (hex[1] == 'x' || hex[1] == 'X'))
            hex = hex.substr(2);
        ...
        for (int i = 0; i < CELLS; ++i) {
            ...
            if (board[i] > 10) return -4;
            if ((target == 512 && board[i] >= 9) || (target == 1024 && board[i] >= 10))
                already_won = true;
        }
...
int main(int argc, char **argv) {
	try {
		...
		if (target != 512 && target != 1024) throw runtime_error("target must be 512 or 1024");
		if (model.empty()) model = "table_3x3_" + to_string(target) + ".bin";

		string hex = argv[arg++];
		if (hex.size() == 11 && hex[0] == '0' && (hex[1] == 'x' || hex[1] == 'X')) hex = hex.substr(2);
		if (hex.size() != CELLS) throw runtime_error("board must be exactly 9 hex digits");
		array<uint8_t, CELLS> board{};
		bool already_won = false;
		for (int i = 0; i < CELLS; ++i) {
			board[i] = hex_exp(hex[i]);
			if (board[i] > 10) throw runtime_error("hex digit cannot exceed A for 512/1024 table");
			if ((target == 512 && board[i] >= 9) || (target == 1024 && board[i] >= 10)) already_won = true;
		}
