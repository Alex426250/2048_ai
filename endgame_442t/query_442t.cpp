...
#define T442_PREFETCH(p) __builtin_prefetch((const void *)(p))
#elif defined(_MSC_VER)
#include <intrin.h>
#define T442_PREFETCH(p) _mm_prefetch((const char *)(p), _MM_HINT_T0)
#else
#define T442_PREFETCH(p) ((void)0)
#endif
...
struct Table {
    MappedFile mf;
    T442Header h{};
    const T442LayerDir *dir = nullptr;
    ...
    void load(const string &file, uint32_t target) {
        mf.open_file(file);
        if (mf.size < sizeof(T442Header)) throw runtime_error("model too small: " + file);
        memcpy(&h, mf.data, sizeof(h));
        if (memcmp(h.magic, "442tWIN", 7) != 0 || h.version != 1 || h.rows != 4 || h.cols != 4 ||
            h.free_count != FREE_N || h.cfg_count != NCFG || h.value_bytes != 3 || 
            (h.key_bytes != 4 && h.key_bytes != 8) ||
            h.prob_scale == 0 || h.file_size != mf.size)
            throw runtime_error("bad 442t model: " + file);
        ...
        if (h.layer_dir_off + (uint64_t)h.layer_count * sizeof(T442LayerDir) > mf.size) throw runtime_error("truncated layer directory: " + file);
        dir = (const T442LayerDir *)(mf.data + h.layer_dir_off);
        pw[0] = 1;
    ...
    double lookup(uint64_t key, int layer) const {
        ...
        const T442LayerDir &ld = dir[layer];
...
#ifdef _WIN32
#define T442_EXPORT __declspec(dllexport)
#else
#define T442_EXPORT __attribute__((visibility("default")))
#endif
...
extern "C" T442_EXPORT int init_model(const char *model_path, int target) {
    try {
        Table *tab = new Table();
...
extern "C" T442_EXPORT int query_probs(const char *board_hex, int target, double out_probs[4]) {
...
extern "C" T442_EXPORT int query_state(const char *board_hex, int target, double *out) {
    const Table *t = pick_table(target);
    if (!t) return -2;
    const Table &tab = *t;
    try {
...
extern "C" T442_EXPORT void free_models() {
...
void usage(const char *a0) {
    fprintf(stderr,
        ...
        "      恰好 6 个数码必须 >= 目标指数，并组成允许的 442t 大数配置。\n"
        "例：  %s 512 0000000000AAAAAA\n", a0, a0, a0, a0);
}
